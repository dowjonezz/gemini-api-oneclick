...
# Current Gemini Web /usage request shape. These values are not credentials;
# they mirror the browser feature/fingerprint headers required by this RPC.
_USAGE_MODEL_HEADER = [
    1,
    None,
    None,
...
_USAGE_BROWSER_HEADERS = {
    ...
    "Sec-CH-UA-Full-Version-List": (
        '"Chromium";v="152.0.7977.64", "Not?A_Brand";v="24.0.0.0", '
        '"Google Chrome";v="152.0.7977.64"'
    ),
    "Sec-CH-UA-Mobile": "?0",
    "Sec-CH-UA-Model": '""',
...
def _batch_headers_for_usage(client: Any) -> dict[str, str]:
    ...
    if not batch_session_id:
        batch_session_id = _usage_session_ids.setdefault(
            key,
            str(uuid.uuid4()).upper(),
        )

    model_header = list(_USAGE_MODEL_HEADER)
...
def session_snapshot(slot: Any) -> dict[str, Any]:
    ...
    for session_id, record in list(getattr(store, "sessions", {}).items()):
        ...
        cid = (
            getattr(chat, "cid", "")
            or getattr(getattr(chat, "_chat", None), "cid", "")
            or ""
        )
        items.append({
